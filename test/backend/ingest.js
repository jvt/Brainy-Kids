        ...
        .then(function(teacher_results) {
            ...
            createProgramsRandom(program_jsons, token)
                .then(function(programs) {
                    createStudentsRandom(
                        number_of_students,
                        teacher_docs,
                        tokens
                    )
                        .then(function(students) {
                            createFocusItemsRandom(
                                number_of_focus_items,
                                programs,
                                token
                            )
                                .then(function(focus_items) {
                                    createAnalyticsRandom(
                                        number_of_analytics,
                                        students,
                                        focus_items,
                                        tokens
...
async function createAnalyticsRandom(
    number_of_analytics,
    student_docs,
    focus_item_docs,
    teacher_tokens
    ...
    for (student of students) {
        for (focus_item of focus_items) {
            ...
            var analytic_json = {
                ...
                correct_on: correct_on,
                time_spent:
                    correct_on * Math.floor(Math.random() * 61000) + 1000,
            };
            res = await request(app)
                .post('/api/analytics/application')
                .send(analytic_json)
